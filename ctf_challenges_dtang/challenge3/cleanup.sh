...
for c in $(docker ps -aq --filter "ancestor=$IMAGE_NAME"); do docker rm -f "$c" >/dev/null || true; done

echo "[*] Removing staged files..."
rm -f "$HERE/Dockerfile" "$HERE/flag.txt" "$HERE/entrypoint.sh"
rm -f "$HERE/app/leak_once.sh" "$HERE/app/.bashrc" "$HERE/app/sudoers.d/ctfuser"
